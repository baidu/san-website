...
Specify how the view updates. Currently only `"optimized"` is supported and when specified, the `s-for` directive updates the view according to the specific browser environment, during which the DOM manipulations may not correspond to the data changes.
...
The parse result of `.template`. To speed up initialization, the template will not be compiled again during component instantiation if the `.aNode` property already exists. `.aNode` is used by pre-compile optimization and typically generated by build tools, so you won't need it in your source code. Use [san-anode-utils](https://github.com/ecomfe/san-anode-utils) to work with [ANode](https://github.com/baidu/san/blob/master/doc/anode.md).
